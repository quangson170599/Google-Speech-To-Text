...
def GCP_s2t(alist):
    ...
    def thread_function(index,file_addr,config):
        try:
            text=Transcribe_Short_Audio(file_addr,config)
        except:
            text = Transcribe_Long_Audio(file_addr,config)
        x = index,text
        return x
    ...
    for adict in alist:
        for speaker,value in adict.items():
                ...
                with ThreadPoolExecutor(8) as executor:
                    futures = executor.map(thread_function,index,file_addr,config)
                    index_text = [x for x in futures]
                index_text.sort(key=func_sort)
                text = [text for _,text in index_text]

                with ThreadPoolExecutor(8) as executor:
                    futures2 = executor.map(thread_function,index,file_addr,config_noPunc)
...
def Handle_Dict(adict,S_Punc_ok=False,XD=False):
    adict = list(adict.values())
    Spr,SL = 0,0
    ReDict = {'Speaker':[], 'co dau':[],'khong dau':[]}
    if XD == True:
...
def split_silence(audio, speaker, silence_time=300,time_ok=None):
